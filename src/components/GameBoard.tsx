...
export default function GameBoard({ socket, state, hand }: GameBoardProps) {
  ...
  return (
    <div
      className={
        compactLayout
          ? "flex flex-col flex-1 min-h-0 h-full w-full px-1"
          : "flex gap-2 sm:gap-4 lg:gap-6 items-start justify-center w-full max-w-7xl mx-auto px-1.5 sm:px-2 py-1 sm:py-2 sm:p-4"
      }
    >
      {/* Scoreboard (desktop) */}
      ...
      <div
        className={
          compactLayout
            ? "grid grid-rows-[auto_minmax(0,1fr)_auto] flex-1 min-h-0 w-full gap-1.5"
            : "flex-1 flex flex-col items-center gap-2 sm:gap-4 min-h-0 w-full"
        }
      >
        {/* Mobile score strip */}
        <div className="lg:hidden w-full shrink-0 flex flex-col gap-0.5 bg-white/[0.04] rounded-lg border border-white/[0.08] px-2 py-1.5 text-sm">
          <div className="flex items-center justify-between gap-1">
            ...
            <div className="text-[9px] text-zinc-500 text-center leading-snug min-w-0 flex-1 px-1">
        ...
        <div
          className={
            compactLayout
              ? "relative min-h-0 w-full max-w-2xl mx-auto felt-bg rounded-xl border border-emerald-700/20 shadow-xl shadow-black/40 overflow-hidden row-start-2"
              : "relative w-full max-w-2xl mx-auto aspect-square felt-bg rounded-2xl sm:rounded-3xl border border-emerald-700/20 shadow-2xl shadow-black/50 overflow-visible"
          }
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(16,185,129,0.06),transparent_70%)]" />
          <div className="absolute inset-2 sm:inset-5 rounded-lg sm:rounded-2xl border border-emerald-600/10" />
      ...
      {showOverlay &&
        (compactLayout ? (
          <div
            className="absolute inset-0 bg-black/65 flex items-center justify-center z-30"
            role="presentation"
          >
            <HandResultDialog
              state={state}
        ...
        {/* Status bar — desktop only; mobile uses trick-area message */}
        {!compactLayout ? (
...
function HandResultDialog({
  ...
  return (
    <div
      ...
      className="bg-zinc-900/95 border border-white/[0.1] rounded-2xl p-5 sm:p-8 text-center w-[min(calc(100vw-1.5rem),24rem)] max-h-[85dvh] overflow-y-auto overscroll-contain shadow-2xl mx-2"
    >
      {state.phase === "hand_complete" && (
        <>
          ...
          <button
            type="button"
            onClick={onNextHand}
            className="w-full min-h-[48px] px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold rounded-xl shadow-lg shadow-emerald-500/20 active:scale-[0.98]"
          >
            Next Hand
          </button>
        </>
      ...
      {state.phase === "game_over" && (
        <>
          ...
          <button
            type="button"
            onClick={onNewGame}
            className="w-full min-h-[48px] px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-bold rounded-xl shadow-lg shadow-amber-500/20 active:scale-[0.98]"
          >
            Play Again
          </button>
        </>
