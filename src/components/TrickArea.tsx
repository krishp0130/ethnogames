...
export default function TrickArea({
  ...
  const size: CardSize = isCompact ? "sm" : "lg";
  const layout = TRICK_LAYOUTS[size];

  const trumpBadge = trumpRevealed && trumpSuit && (
    isCompact ? (
      <div className="absolute -top-7 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-amber-500/90 text-white px-2.5 py-1 rounded-full text-[10px] font-bold shadow-lg shadow-amber-500/30 z-10 whitespace-nowrap">
        <span>Trump</span>
        <span className="text-sm leading-none">{SUIT_SYMBOLS[trumpSuit]}</span>
      </div>
    ) : (
      <motion.div
  ...
  const trickCards = currentTrick.map((tc) => {
  ...
  return (
    <div className={`relative ${layout.container}`}>
      {isCompact ? trumpBadge : (
        <AnimatePresence>{trumpBadge}</AnimatePresence>
      )}

      {isCompact ? trickCards : (
        <AnimatePresence mode="popLayout">{trickCards}</AnimatePresence>
      )}

      {currentTrick.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center px-3 sm:px-4">
          <p className="text-white/35 text-[11px] sm:text-sm text-center font-medium leading-snug line-clamp-3">
            {message}
          </p>
        </div>
      )}
