...
function GameCard({
  ...
  const cardClass = `
    group block p-6 rounded-2xl border h-full
    ${available
      ? "bg-gradient-to-br from-amber-500/[0.08] to-orange-500/[0.04] border-amber-500/20 hover:border-amber-500/40 hover:shadow-lg hover:shadow-amber-500/10 transition-[border-color,box-shadow]"
  ...
  return (
    ...
    >
      {available ? (
        <Link href={href} className={`${cardClass} interactive-focus`}>
          {inner}
        </Link>
      ) : (
